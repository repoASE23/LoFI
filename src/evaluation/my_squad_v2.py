...
from evaluation.my_evaluator import  (
...
class SquadV2(evaluate.Metric):
    ...
    def _compute(self, predictions, references, no_answer_threshold=1.0):
        ...
        exact_raw, f1_raw, pre_raw, rec_raw = get_raw_scores(dataset, predictions)
        ...
        pre_thresh = apply_no_ans_threshold(pre_raw, no_answer_probabilities, qid_to_has_ans, no_answer_threshold)
        rec_thresh = apply_no_ans_threshold(rec_raw, no_answer_probabilities, qid_to_has_ans, no_answer_threshold)
        out_eval = make_eval_dict(exact_thresh, f1_thresh, pre_thresh, rec_thresh)
        # print(out_eval)
        if has_ans_qids:
            has_ans_eval = make_eval_dict(exact_thresh, f1_thresh, pre_thresh, rec_thresh, qid_list=has_ans_qids)
            merge_eval(out_eval, has_ans_eval, "HasAns")
        if no_ans_qids:
            no_ans_eval = make_eval_dict(exact_thresh, f1_thresh, pre_thresh, rec_thresh, qid_list=no_ans_qids)
            merge_eval(out_eval, no_ans_eval, "NoAns")
        find_all_best_thresh(out_eval, predictions, exact_raw, f1_raw, pre_raw, rec_raw, no_answer_probabilities, qid_to_has_ans)
