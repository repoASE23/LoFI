    ...
    for article in dataset:
        for p in article["paragraphs"]:
            for qa in p["qas"]:
                ...
                precision_scores[qid] = max(compute_precision(a, a_pred) for a in gold_answers)
                recall_scores[qid] = max(compute_recall(a, a_pred) for a in gold_answers)
...
def make_eval_dict(exact_scores, f1_scores, pre_scores, rec_scores, qid_list=None):
    if not qid_list:
        ...
        return collections.OrderedDict(
            [
                ...
                ("precision", 100.0 * sum(pre_scores.values()) / total),
                ("recall", 100.0 * sum(rec_scores.values()) / total),
                ("total", total),
            ]
        )
    else:
...
def find_all_best_thresh(main_eval, preds, exact_raw, f1_raw, pre_raw, rec_raw, na_probs, qid_to_has_ans):
    best_exact, exact_thresh = find_best_thresh(preds, exact_raw, na_probs, qid_to_has_ans)
    best_f1, f1_thresh = find_best_thresh(preds, f1_raw, na_probs, qid_to_has_ans)
    best_pre, pre_thresh = find_best_thresh(preds, pre_raw, na_probs, qid_to_has_ans)
    best_rec, rec_thresh = find_best_thresh(preds, rec_raw, na_probs, qid_to_has_ans)
    main_eval["best_exact"] = best_exact
    main_eval["best_exact_thresh"] = exact_thresh
    main_eval["best_f1"] = best_f1
    main_eval["best_f1_thresh"] = f1_thresh
    main_eval["best_pre"] = best_pre
    main_eval["best_pre_thresh"] = pre_thresh
    main_eval["best_rec"] = best_rec
    main_eval["best_rec_thresh"] = rec_thresh
